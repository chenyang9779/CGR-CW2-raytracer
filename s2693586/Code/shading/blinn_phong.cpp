...
// Function for Blinn-Phong shading with BVH acceleration
Vector3 blinnPhongShadingBVH(const Intersection &intersection, const Ray &ray, const std::vector<Light> &lights, const BVHNode *root, int nbounces, const Vector3 &backgroundColor) {
    ...
    // Direct illumination using Blinn-Phong model
    for (const auto &light : lights) {
        Vector3 lightDir = (light.position - intersection.point).normalize();
        float distanceToLight = (light.position - intersection.point).length();
        ...
        bool inShadow = root->intersect(shadowRay, shadowIntersection) && shadowIntersection.distance < distanceToLight;
    ...
    if (material.isReflective) {
        ...
        if (root->intersect(reflectionRay, closestReflectionIntersection)) {
            Vector3 reflectionColor = blinnPhongShadingBVH(closestReflectionIntersection, reflectionRay, lights, root, nbounces - 1, backgroundColor);
            color = (1.0f - material.reflectivity) * color + material.reflectivity * reflectionColor;
        }
    }
    ...
    if (material.isRefractive) {
        ...
        if (calculateRefraction(ray.direction, normal, material.refractiveIndex, refractionDir)) {
            ...
            if (root->intersect(refractionRay, closestRefractionIntersection)) {
                Vector3 refractionColor = blinnPhongShadingBVH(closestRefractionIntersection, refractionRay, lights, root, nbounces - 1, backgroundColor);
                color = (1.0f - fresnelReflectance) * refractionColor + fresnelReflectance * color;
            }
        }
    }
